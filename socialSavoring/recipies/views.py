...
def recipe_update_view(request, id):
  ...
  if request.method == 'POST':
    ...
    recipe_tags = request.POST.get('recipe_tags', '')
    recipe_instructions = request.POST.get('recipe_instructions', '')
    Recipe.objects.filter(pk=id).update(updated_timestamp=updated_timestamp, recipe_name=recipe_name, oven_temp=oven_temp, serving_size=serving_size, recipe_category=recipe_category, recipe_tags=recipe_tags, recipe_instructions=recipe_instructions)
